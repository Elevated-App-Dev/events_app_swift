...
class AchievementSystem {
    ...
    private static let definitions: [AchievementType: AchievementDefinition] = [
        ...
        // Secret
        .aboveAndBeyond:       AchievementDefinition(type: .aboveAndBeyond, name: "Above and Beyond", description: "Trigger hidden reputation bonuses 10 times", hiddenDescription: "Go the extra mile...", category: .secret, targetProgress: 10, isSecret: true),
        .familyFirst:          AchievementDefinition(type: .familyFirst, name: "Family First", description: "Never use family emergency funds", hiddenDescription: "Stand on your own two feet", category: .secret, isSecret: true),
        .comebackKid:          AchievementDefinition(type: .comebackKid, name: "Comeback Kid", description: "Recover from a Financial Crisis", hiddenDescription: "What doesn't kill you...", category: .secret, isSecret: true)
    ]
    ...
    private func initializeAchievements() {
        ...
        for (_, def) in Self.definitions {
            achievements[def.type] = AchievementData(from: def)
        }
    }
    ...
    func incrementProgress(_ achievement: AchievementType, by amount: Int) {
    ...
    func getProgress(_ achievement: AchievementType) -> AchievementProgress {
        guard let data = achievements[achievement] else {
            return AchievementProgress(type: achievement, current: 0, target: 1, isComplete: false)
        }
        return AchievementProgress(type: achievement, current: data.currentProgress, target: data.targetProgress, isComplete: data.isEarned)
    }
    ...
    func getSaveData() -> AchievementSaveData {
        ...
        for (type, data) in achievements {
            if data.isEarned {
                ...
                if let earnedTime = data.earnedTime {
                    save.earnedTimestamps.append(earnedTime)
                }
            }
